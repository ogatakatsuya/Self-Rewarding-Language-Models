...
class ConfigLoader:
  ...
  def setup_paths(self):
    ...
    """
    base_path = Path(
      os.getenv("PROJECT_BASE_PATH", Path(__file__).resolve().parent.parent.parent)
    )
    self.logger.info(f"Base path set to: {base_path}")
    self.config["ift_data_path"] = (base_path / self.config["data_directory"]).resolve()
    ...
    self.logger.info(f"Model directory path set to: {self.config['model_dir_path']}")
  ...
  def setup_experiment_dir(self):
    ...
    try:
      ...
      self.logger.info(f"Experiment directory set to: {self.config['experiment_dir']}")
