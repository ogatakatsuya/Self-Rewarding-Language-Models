...
from src.utils.logging.color_formatter import ColoredFormatter


def setup_logging(
  default_path: str = "src/utils/logging/logging_config.json",
    ...
    if os.path.exists(path):
      ...
      for _, handler_details in config["handlers"].items():
        if "formatter" in handler_details:
          ...
          formatter = ColoredFormatter(fmt=formatter_config["format"])
